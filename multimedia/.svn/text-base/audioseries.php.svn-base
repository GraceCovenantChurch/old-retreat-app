...
EOF;
    ...
    $qry = "SELECT Type, DATE_FORMAT(`Date`,'%m.%d.%y'), Title, MP3, M3U, Pdf, BibleBooks.Book, ch_start, ch_end, vs_start, vs_end, Speakers.Name, Extra FROM Sermons, BibleBooks, Speakers WHERE Series = $ser AND BibleBooks.Num = Sermons.Book AND Speakers.ID = Speaker ORDER BY `Date` DESC";
    $res = mysql_query($qry);
    ...
    {
	list($type, $date, $title, $mp3, $m3u, $pdf, $book, $ch1, $ch2, $vs1, $vs2, $spk, $extra) = $row;
	if ($last_series != 0) {
	    echo <<<EOF
...
EOF;
	...
	$file_line = "";
	if ($mp3) {
	    $file_line .= "<a href=\"audiofiles/$mp3\" " . getGoogleAnalyticsStr($date) . ">file</a>";
	    if ($m3u) { $file_line .= " | "; }
	}
	if ($m3u) {
	    $file_line .= "<a href=\"audiofiles/$m3u\" " . getGoogleAnalyticsStr($date) . ">stream</a>";
	}
        if ($pdf) {
            $extensions = explode('.', $pdf);
            $extension = $extensions[1];
	    $file_line .= " | <a href='http://gracecovenant.net/resources/audiosermons/sermonFiles/" . $pdf . "'>" . $extension . "</a>";
        }
	...
	echo <<<EOF
	<tr align="center" class="text"> 
	  <td height=28 bgcolor="#F0F0F0" class="text"><strong><font class="$type">$typeText</strong></td>
