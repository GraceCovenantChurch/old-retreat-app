...
function echo_series($ser)
{
    ...
    $qry = "SELECT Series, Type, DATE_FORMAT(`Date`,'%m.%d.%y'), Title, MP3, M3U, Pdf, BibleBooks.Book, ch_start, ch_end, vs_start, vs_end, Speakers.Name, Extra FROM Sermons, BibleBooks, Speakers WHERE Series = $ser AND BibleBooks.Num = Sermons.Book AND Speakers.ID = Speaker ORDER BY `Date` DESC";
    $res = mysql_query($qry);
    if ($res === false) {
...
EOF;
    ...
    $qry = "SELECT Series, Type, DATE_FORMAT(`Date`,'%m.%d.%y'), Title, MP3, M3U, Pdf, BibleBooks.Book, ch_start, ch_end, vs_start, vs_end, Speakers.Name, Extra FROM Sermons, BibleBooks, Speakers WHERE YEAR(`Date`) = $year AND BibleBooks.Num = Sermons.Book AND Speakers.ID = Speaker $filt ORDER BY `Date` DESC";
    $res = mysql_query($qry);
    $series_used = array();
    $last_series = -1;
    if (!$res) {
